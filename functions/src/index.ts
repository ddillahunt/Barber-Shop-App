...
function getRecipient(data: EmailData): { email: string; name?: string } {
  switch (data.type) {
    ...
    case "reminder":
    case "contact_reply":
      if (!data.email) throw new HttpsError("invalid-argument", "Customer email is required");
...
export const sendEmail = onCall({ secrets: [brevoApiKey] }, async (request) => {
  ...
  logger.info("Sending email via Brevo", { type: data.type, to: recipient.email });
  ...
  logger.info("Email sent successfully", { type: data.type, to: recipient.email });
  return { success: true };
});
...
export const sendSMS = onCall({ secrets: [brevoApiKey] }, async (request) => {
  ...
  logger.info("Sending SMS via Brevo", { to: recipient });
...
export const sendAppointmentReminders = onSchedule(
  ...
  async () => {
    ...
    logger.info("Checking for appointments to remind", { today, now: now.toISOString(), oneHourFromNow: oneHourFromNow.toISOString() });
    ...
    for (const doc of snapshot.docs) {
      ...
      try {
        ...
        logger.info("Reminder sent", { name: appt.name, email: appt.email, time: appt.time });
      } catch (err) {
        logger.error("Failed to send reminder", { name: appt.name, error: String(err) });
      }
    }
